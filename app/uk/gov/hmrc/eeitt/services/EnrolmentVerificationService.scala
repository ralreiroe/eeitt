...
trait EnrolmentVerificationService {
  ...
  def verify(enrolmentRequest: RegisterRequest): Future[RegistrationResponse] =
  ...
  def verifyAgent(enrolmentRequest: RegisterAgentRequest): Future[RegistrationResponse] =
    enrolmentRepo.lookupEnrolment(??? /* enrolmentRequest.registrationNumber*/ ).flatMap {
      case Nil => Future.successful(RESPONSE_NOT_FOUND)
      case x :: Nil => doVerify(??? /* enrolmentRequest */, x)
      case x :: xs => Future.successful(MULTIPLE_FOUND)
  ...
  private def doVerify(request: RegisterRequest, enrolment: Enrolment): Future[RegistrationResponse] =
  ...
  private def incorrectArnResponse(request: RegisterRequest): Future[RegistrationResponse] =
    request.arn match {
  ...
  object DifferentPostcodes {
    def unapply(p: (RegisterRequest, Enrolment)): Boolean = p match {
